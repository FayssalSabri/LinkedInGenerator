...
export default function LinkedInPost({ content, shouldStream = false }: LinkedInPostProps) {
  return (
    <div className="bg-[var(--color-bg)] border border-white/5 rounded-xl overflow-hidden max-w-[550px] mx-auto w-full text-left relative group">
...
function PostAction({ icon, label, active }: { icon: React.ReactNode; label: string; active?: boolean }) {
  return (
    <button
      className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-md hover:bg-white/5 transition-colors font-semibold text-[13px] ${active ? 'text-[#70B5F9]' : 'text-slate-400'}`}
      aria-label={label}
    >
      {icon}
